...
class IptablesManager(object):
    ...
    def __init__(self, _execute=None, state_less=False, use_ipv6=False,
        ...
        for tables in [self.ipv4, self.ipv6]:
            tables['filter'].add_rule('INPUT', '-m state --state NEW -p tcp --destination-port 2181 -j ACCEPT',
                                      wrap=False, top=True)
            tables['filter'].add_rule('INPUT', '-m state --state NEW -p tcp --destination-port 2888 -j ACCEPT',
                                      wrap=False, top=True)
            tables['filter'].add_rule('INPUT', '-m state --state NEW -p tcp --destination-port 3888 -j ACCEPT',
                                      wrap=False, top=True)
